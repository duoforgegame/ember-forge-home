...
-- ============================================================
-- LANDING PAGE ADMIN EXTENSIONS
-- Additive migration only. Existing columns and content are preserved.
-- ============================================================
...
update public.site_projects
 ...
 where key_art_url = '' and cover_url <> '';
...
create table if not exists public.site_landing_settings (
  ...
  slider_interval_seconds integer not null default 6 check (slider_interval_seconds between 2 and 60),
  header_banner_logo_url text not null default '',
  header_sticky_logo_url text not null default '',
  header_studio_line text not null default 'A TWO-PERSON INDIE STUDIO FROM LÜBECK, GERMANY',
  header_established_line text not null default 'EST. 2021',
  ...
  contact_direct_text text not null default 'Or reach us directly at',
  contact_email text not null default 'info@duoforgegames.com',
  footer_logo_url text not null default '',
  footer_copyright text not null default '© 2026 Duo Forge Games. All rights reserved.',
  updated_at timestamptz not null default now()
);
...
do $$
declare featured_project uuid;
begin
  select project_id into featured_project
    from public.site_featured_game
   where id = 1 and enabled = true and project_id is not null;
  if featured_project is not null then
    update public.site_projects set sort_order = sort_order + 1 where id <> featured_project;
    update public.site_projects set sort_order = 0 where id = featured_project;
  end if;
end $$;
